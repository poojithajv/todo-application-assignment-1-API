...
const convertTodoDBObjectAPI1 = (object) => {
  return {
    id: object.id,
    todo: object.todo,
    ...
    case hasPriorityAndStatusProperties(request.query):
      if (priority === "HIGH" || priority === "MEDIUM" || priority === "LOW") {
        if (
          ...
          response.send(
            dbResponse.map((object) => convertTodoDBObjectAPI1(object))
          );
        } else {
          response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(convertTodoDBObjectAPI1(getTodoResponse));
});

//GET todo list with specific due date
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    response.send(
      responseResult.map((object) => convertTodoDBObjectAPI1(object))
    );
  } else {
    response.status(400);
